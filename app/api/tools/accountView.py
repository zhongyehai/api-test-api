...
class AccountView(BaseMethodView):
    ...
    def post(self):
        ...
        if AccountModel.get_first(account=request.json['account']):
            return restful.fail('账号已存在，直接修改即可')
        with db.auto_commit():
    ...
    def put(self):
        """ 修改账号 """
        # 账号不重复
        account = AccountModel.get_first(account=request.json.get('account'))
        if account and account.id != request.json.get('id'):
            return restful.fail(f'账号：{account.aaccount} 已存在', data=account.to_dict())
