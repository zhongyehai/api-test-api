...
class Task(BaseModel):
    ...
    name = db.Column(db.String(64), comment='任务名称')
    cron = db.Column(db.String(256), nullable=True, comment='cron表达式')
    set_id = db.Column(db.String(2048), comment='用例集id')
    case_id = db.Column(db.String(2048), comment='用例id')
    is_send = db.Column(db.String(16), comment='是否发送报告，1.不发送、2.始终发送、3.仅用例不通过时发送')
    send_type = db.Column(db.String(16), default='webhook', comment='测试报告发送类型，webhook，email，all')
    webhook = db.Column(db.String(2048), comment='企业微信或钉钉webhook地址')
    task_type = db.Column(db.String(16), default='cron', comment='定时类型')
    email_server = db.Column(db.String(256), comment='发件邮箱服务器')
    email_to = db.Column(db.String(256), comment='收件人邮箱')
    email_from = db.Column(db.String(256), comment='发件人邮箱')
    email_pwd = db.Column(db.String(256), comment='发件人邮箱密码')
    status = db.Column(db.String(16), default=u'禁用中', comment='任务的运行状态，默认是禁用中')
